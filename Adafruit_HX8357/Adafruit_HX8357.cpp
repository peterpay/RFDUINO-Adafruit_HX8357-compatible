...
void Adafruit_HX8357::spiwrite(uint8_t c) {
    ...
    for (i=7; i>=0; i--){
        if (c & _BV(i)) {
            NRF_GPIO->OUTSET = (1 << _mosi);
        } else {
            NRF_GPIO->OUTCLR = (1 << _mosi);
        }
    }
}
*/
...
void Adafruit_HX8357::spiwrite(uint8_t c) {
	if (hwSPI) {		
	    ...
	    for(uint8_t bit = 0x80; bit; bit >>= 1) {
	      if(c & bit)  NRF_GPIO->OUTSET = (1 << _mosi); 
	      else         NRF_GPIO->OUTCLR = (1 << _mosi);
...
/* commented to test optimization

void Adafruit_HX8357::spiwrite(uint8_t c) {
	if (hwSPI) {
		//SPI.setFrequency(16); 
	    SPI.setClockDivider(64); // 8-ish MHz (full! speed!)
  ...
  if (hwSPI) {
    ...
    SPI.setClockDivider(21); // 8-ish MHz (full! speed!)
    SPI.setBitOrder(MSBFIRST);
    SPI.setDataMode(SPI_MODE0);
    SPI.transfer(c);
#endif
...
#if defined (__AVR__) || defined (TEENSYDUINO)
    SPI.begin();
    SPI.setClockDivider(1); //SPI_CLOCK_DIV2 8 MHz (full! speed!)
    SPI.setBitOrder(MSBFIRST);
    SPI.setDataMode(SPI_MODE0);
    mySPCR = SPCR;
#elif defined (__arm__)
    SPI.begin();
    SPI.setClockDivider(1); // 8-ish MHz (full! speed!)
    SPI.setBitOrder(MSBFIRST);
    SPI.setDataMode(SPI_MODE0);
#endif
  } else {
...
void Adafruit_HX8357::sendbyte(uint8_t data){
  uint8_t i = 0;
...
void Adafruit_HX8357::sendbyteLSB(uint8_t data){
  uint8_t i = 0;
...
void Adafruit_HX8357::fillScreenFast(int16_t x, int16_t y, int16_t w, int16_t h,uint16_t color) {
    uint8_t hi = color >> 8, lo = color;
    digitalWrite(_dc, HIGH);
    digitalWrite(_cs, LOW);
    for(y=h; y>0; y--) {
        drawFastHLine(x,y,w,h);
    }
}
